...
def save_memory(memory):
    ...
    try:
        ...
        _bl_data = {
            "blocked_longs": _auto_blocked,
            "updated_at":    bkk_now_str(),
            "note": "Auto-generated by debrief — coins with ≥3 LONG losses + 0 LONG wins",
        }
        with open(_bl_path, "w", encoding="utf-8") as _blf:
            json.dump(_bl_data, _blf, indent=2)
        if _auto_blocked:
            log(f"   🚫 AUTO-BLOCKLIST updated: {', '.join(_auto_blocked)} ({len(_auto_blocked)} coins)")
